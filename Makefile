...
CRYSTAL = crystal
SHARDS = shards
CRFLAGS = --stats --cross-compile --prelude=./prelude --target="$(TARGET_TRIPLET)" --error-trace --emit llvm-ir -Dgc_none
...
qemu-debug: $(NAME).iso
	qemu-system-i386 -d cpu_reset -d int -serial mon:stdio -machine q35 -no-reboot -m 512 -boot d -cdrom $(NAME).iso -vnc ${VNC_PORT} -s -S

qemu: $(NAME).iso
	qemu-system-i386 -serial stdio -machine q35 -no-reboot -m 512 -boot d -cdrom $(NAME).iso -vnc ${VNC_PORT}
